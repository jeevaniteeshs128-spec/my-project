...
export function MeetupsPage() {
  return (
    <div className='space-y-5'>
      <div>
        <p className='text-xs uppercase tracking-[0.22em] text-[#7C3AED]'>Meetups</p>
      ...
      <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-3'>
        {meetups.map((meetup, index) => {
          ...
          return (
            <article key={meetup.id} className='overflow-hidden rounded-lg border border-white/10 bg-[#162033]'>
              <div className='relative h-44'>
                <img src={movie.backdrop} alt={movie.title} className='absolute inset-0 h-full w-full object-cover' />
                <div className='absolute inset-0 bg-gradient-to-t from-[#162033] via-[#162033]/55 to-transparent' />
              ...
              <div className='space-y-3 p-4'>
                ...
                <div className='grid gap-2 text-sm text-slate-300'>
                  <span className='inline-flex items-center gap-2'><CalendarDays className='h-4 w-4 text-[#7C3AED]' /> {meetup.date} - {meetup.cadence}</span>
                  <span className='inline-flex items-center gap-2'>
                    {meetup.mode === 'offline' ? <MapPin className='h-4 w-4 text-[#7C3AED]' /> : <Video className='h-4 w-4 text-[#7C3AED]' />}
                    {meetup.communityOnly ? 'Community members only' : 'Open to eligible members'}
                  </span>
                </div>
                <button className='w-full rounded-md bg-[#7C3AED] px-3 py-2 text-sm font-semibold text-white transition-colors duration-200 hover:bg-[#9333EA]'>
                  Register
                </button>
              </div>
            </article>
