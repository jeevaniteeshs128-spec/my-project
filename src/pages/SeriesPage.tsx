  ...
  return (
    <div className='space-y-5'>
      <div className='flex flex-wrap items-end justify-between gap-3'>
        <div>
          <p className='text-xs uppercase tracking-[0.22em] text-[#35d07f]'>Series</p>
          <h1 className='mt-1 text-2xl font-semibold text-white sm:text-3xl'>Browse TV and episodes</h1>
        </div>
        <label className='flex h-10 w-full items-center gap-2 rounded-md border border-white/10 bg-white/6 px-3 text-slate-400 focus-within:border-[#35d07f]/60 sm:w-80'>
      ...
      <div className='sticky top-0 z-20 -mx-3 space-y-3 border-y border-white/10 bg-black/88 px-3 py-3 backdrop-blur-xl md:top-[72px] sm:-mx-4 sm:px-4 lg:-mx-6 lg:px-6'>
        <div className='flex gap-2 overflow-x-auto'>
          {filters.map((item) => (
            <button key={item} onClick={() => { setFilter(item); setValue('') }} className={`shrink-0 rounded-md px-3 py-2 text-xs font-medium capitalize ${filter === item ? 'bg-[#35d07f] text-black' : 'bg-white/8 text-slate-200'}`}>
              {item}
            </button>
          ))}
        </div>
        <div className='grid gap-2 sm:grid-cols-[minmax(220px,360px)_auto_1fr] sm:items-center'>
          <select value={value} onChange={(event) => setValue(event.target.value)} className='h-10 rounded-md border border-white/10 bg-[#101827] px-3 text-sm text-white outline-none'>
