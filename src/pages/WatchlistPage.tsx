  ...
  return (
    <div className='space-y-5'>
      <div className='flex flex-wrap items-end justify-between gap-3'>
        <div>
          <p className='text-xs uppercase tracking-[0.22em] text-[#35d07f]'>Watchlist</p>
          ...
          {(['public', 'private'] as const).map((item) => (
            <button
              ...
              className={`rounded px-3 py-2 text-sm capitalize transition-colors duration-200 ${
                currentList === item ? 'bg-[#35d07f] text-[#081023]' : 'text-slate-300 hover:text-white'
              }`}
            >
              {item}
            </button>
      ...
      <div className='grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7'>
        {activeList.map((movie) => (
          <div key={movie.id} className='relative'>
            {currentList === 'private' ? (
              <span className='absolute right-2 top-2 z-10 grid h-7 w-7 place-items-center rounded-full bg-[#081023]/85 text-[#35d07f]'>
