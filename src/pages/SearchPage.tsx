...
export function SearchPage() {
  ...
  return (
    <div className='space-y-5'>
      <div>
        <p className='text-xs uppercase tracking-[0.22em] text-[#7C3AED]'>Universal Search</p>
        <h1 className='mt-1 text-2xl font-semibold text-white sm:text-3xl'>Find movies, series, people and rooms</h1>
      </div>
      <label className='flex h-12 items-center gap-3 rounded-lg border border-white/10 bg-white/[0.065] px-4 text-slate-400 focus-within:border-[#7C3AED]/60'>
      ...
      <div className='flex gap-2 overflow-x-auto'>
        {tabs.map((item) => (
          <button key={item} onClick={() => setTab(item)} className={`shrink-0 rounded-md px-3 py-2 text-xs font-medium ${tab === item ? 'bg-[#7C3AED] text-white' : 'bg-white/8 text-slate-200'}`}>{item}</button>
        ))}
      </div>
      <section className='grid gap-3 md:grid-cols-2'>
        {results.map((item) => (
          <Link key={`${item.mediaKind}-${item.id}`} to={item.mediaKind === 'movie' ? `/movie/${item.id}` : `/series/${item.id}`} className='grid grid-cols-[72px_1fr] gap-3 rounded-lg border border-white/10 bg-white/[0.055] p-3 transition-colors hover:border-[#7C3AED]/60'>
            ...
            <div className='min-w-0'>
              ...
              <p className='mt-2 text-xs text-[#F5B041]'>{item.rating.toFixed(1)} community rating</p>
            </div>
          </Link>
        ))}
      </section>
