...
export function ProfilePage() {
  return (
    <div className='space-y-6'>
      <section className='overflow-hidden rounded-lg border border-white/10 bg-[#162033]'>
        <div className='relative h-48 sm:h-64'>
          <img src={movies[1].backdrop} alt='Profile cover' className='absolute inset-0 h-full w-full object-cover' />
          <div className='absolute inset-0 bg-gradient-to-t from-[#162033] via-[#162033]/45 to-transparent' />
        </div>
        <div className='px-4 pb-4'>
          <div className='-mt-12 flex flex-wrap items-end gap-4'>
            <div className='grid h-24 w-24 place-items-center rounded-md border border-white/15 bg-[#081120] text-2xl font-semibold text-white'>FP</div>
          ...
          <section>
            ...
            <div className='mt-3 grid grid-cols-2 gap-3'>
              {['Martin Scorsese', 'Wong Kar-wai'].map((director) => (
                <div key={director} className='rounded-lg border border-white/10 bg-[#162033] p-3'>
          ...
          <section>
            ...
            <div className='mt-3 space-y-2'>
              {['Best time-loop films', 'Quiet heartbreak dramas', 'Director-first rewatches'].map((list) => (
                <Link key={list} to='/watchlist' className='flex items-center justify-between rounded-lg border border-white/10 bg-[#162033] p-3 text-sm transition-colors duration-200 hover:border-[#7C3AED]/60'>
          ...
          <section>
            ...
            <div className='mt-3 space-y-3'>
              {reviews.slice(0, 3).map((review) => {
                ...
                return (
                  <article key={review.id} className='grid grid-cols-[72px_1fr] gap-3 rounded-lg border border-white/10 bg-[#162033] p-3'>
                    ...
                    <div className='min-w-0'>
                      ...
                      <p className='mt-1 inline-flex items-center gap-1 text-xs text-[#F5B041]'><Star className='h-3 w-3 fill-current' /> {review.rating.toFixed(1)}</p>
